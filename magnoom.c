...
static void magnoom_report_operation(bool success, const char *format, ...)
{
	va_list args;
	va_start(args, format);
	vfprintf(success ? stdout : stderr, format, args);
	va_end(args);
...
static void magnoom_report_read_result(const char *path, bool ok)
{
	magnoom_report_operation(ok, "Reading from the file %s %s!", path, ok ? "succeeded" : "failed");
}
...
void Save_OVF_b8(magnoom_ctx *ctx, double* S, const char *ovf_filename){
    ...
    if(pFile!=NULL) {
        ...
        fputs ("# End: Segment\n",pFile);
        fclose (pFile);
		magnoom_report_operation(true, "Recording to the file %s is done!", ovf_filename);
	} else {
...
void SaveBin(magnoom_ctx *ctx, double* S, const char *bin_filename){
  ...
  fclose (pFile);
   magnoom_report_operation(true, "Recording to the file %s is done!", bin_filename);
}
...
void SavePng(magnoom_ctx *ctx, double* S, const char *png_filename, enSliceMode WhichSliceMode, int x1, int y1, int z1){
  if (WhichSliceMode!=FILTER)
  {
    ...
    if (stbi_write_png(png_filename, width, height, 3, image, width*3))
		magnoom_report_operation(true, "Image has been saved to %s", png_filename);
    else
		magnoom_report_operation(false, "Unable to save PNG image to %s", png_filename);
    free(image);
  }
}
...
void Save_VTS_b4(magnoom_ctx *ctx, double* Sx, double* Sy, double* Sz, float * Px, float * Py, float * Pz, float box[][3], const char *vts_filename){
	...
	magnoom_report_operation(true, "Recording to the file %s is done!", vts_filename);
}
...
void Save_VTK(magnoom_ctx *ctx, double* S, const int mode, const char *vtk_filename)
{
    ...
    if(pFile!=NULL) {
        ...
        fclose (pFile);
		magnoom_report_operation(true, "Recording to the file %s is done!", vtk_filename);
	} else {
...
if(FilePointer!=NULL) {
        ...
        fclose(FilePointer);
		magnoom_report_read_result(vtk_filename, read_ok);
    }else{
        fprintf(stderr, "Cannot open input file '%s': %s\n", vtk_filename, strerror(errno));
		magnoom_report_read_result(vtk_filename, false);
    }
}
