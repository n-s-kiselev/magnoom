                              ...
                              const _GLFWfbconfig *fbconfig )
{
    ...
    // AntTweakBar-Legacy project-specific change: CGDisplayBestModeForParametersAndRefreshRateWithProperty()
    // was removed from the macOS SDK long ago (this file is unmaintained upstream GLFW 2.7.9
    // code from 2011). This block only ran for GLFW_FULLSCREEN, which none of this
    // repository's examples request, so it's disabled rather than reimplemented against a
    // modern display-mode API; width/height simply keep the caller's requested values.
